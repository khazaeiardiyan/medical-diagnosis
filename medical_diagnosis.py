...
Original file is located at
    https://colab.research.google.com/drive/1JBsU7EJSmaIYOB8i7MJWPwA2VXbUIO2T
"""

#Week 1–2
#• clean probability engine
...
#• negative evidence
#• normalization
#• better likelihood model

#Week 5–6
#• Naive Bayes implementation
#• symptom absence handling

#Week 7–8
#• explanation engine (“why this disease”)
#• probability graphs / analysis
...
print(quick_symptoms)

patient_input = input(f'{menu}  :').split(",")

patient_age = "sample"

while patient_age.isdigit() == False:
  patient_age = input("how old are you?  ")
patient_age = int(patient_age)

while patient_age > 99 or patient_age < 0:
  print("age is not within the accepted range of the program.")
  patient_age = input("how old are you? (within range of 0-99)  ")
  patient_age = int(patient_age)

if patient_age >= 90 and patient_age <= 99:
  patient_age = "very elderly"
elif patient_age >= 80 and patient_age <=89:
  patient_age = "elderly"
elif patient_age >= 70 and patient_age <=79:
  patient_age = "late seniors"
elif patient_age >= 60 and patient_age <=69:
  patient_age = "mid-seniors"
elif patient_age >= 50 and patient_age <=59:
  patient_age = "early seniors"
elif patient_age >= 40 and patient_age <=49:
  patient_age = "older adults"
elif patient_age >= 30 and patient_age <=39:
  patient_age = "mid-adults"
elif patient_age >= 20 and patient_age <=29:
  patient_age = "young adults"
elif patient_age >= 15 and patient_age <=19:
  patient_age = "youth"
elif patient_age >= 10 and patient_age <=14:
  patient_age = "children"
elif patient_age >= 5 and patient_age <=9:
  patient_age = "toddler"
elif patient_age >= 0 and patient_age <=4:
  patient_age = "newborn"

patient_sex = "sample"

while patient_sex not in ["male", "female", "other"]:
  patient_sex = input("What is your biological sex? (male/female/other)  ")

patient_symptoms = []

all_sym_ids = list(symptoms.keys())

for number in patient_input:
  if number.isdigit() == True:
    index = int(number)
    if index > -1 and index < 60:
      patient_symptoms.append(all_sym_ids[index])
    else:
      print(f"symptom {index} not recognized, skipping...")
  else:
    print(f"symptom {number} is not recegnized, skipping...")
...
if not patient_symptoms:
  print("no valid symptom found :(")
else:
  print(patient_symptoms)
  if len(patient_symptoms) <= 2:
    print("Low confidence diagnosis — please enter more symptoms for accuracy")
  ...
  for disease_id, disease_data in diseases.items():
    ...
    if disease_name in profile_adjustments:
      ...
      if patient_sex in adjustments:
        prior *= adjustments[patient_sex]

      if patient_age in adjustments:
        prior *= adjustments[patient_age]



    disease_symptoms = disease_data["symptoms"]   # dictionary
    ...
    likelihood = likelihood / len(disease_symptoms)  # normalize
  ...
  for disease, data in result.items():
    all_posterior.append(data["posterior"])
  posterior_1 = softmax(np.array(all_posterior))
  posterior_1 = posterior_1.tolist()

  for disease, data in result.items():
    all_disease_name.append(disease)

  ranking = zip(all_disease_name, posterior_1)
  ranking_sorted = sorted(ranking, key=lambda x : x[1], reverse=True)[:3]

  for index, data in enumerate(ranking_sorted):
    print(f"{index+1}. {data[0]} — {data[1]*100:.1f}%")

    if data[0] in emergency_diseases:
        if index == 0:
            print(f"⚠️ {data[0]} is an emergency! not funny at all :(")

        elif index == 1:
            print(f"⚠️ {data[0]} could be an emergency")

        elif index == 2:
            print(f"⚠️ there is a chance this could be an emergency")